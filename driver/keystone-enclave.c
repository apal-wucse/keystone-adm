...
unsigned long calculate_required_pages(
    unsigned long eapp_sz, unsigned long eapp_stack_sz, unsigned long rt_sz,
    ...
    // For now, we must allocate at least 1 (top) + 2 (enclave) + 2 (runtime)
    // pages for pg tables
    req_pages += 15;
    return req_pages;
}
...
struct enclave *create_enclave(unsigned long min_pages) {
    ...
    enclave->map_target = TRUSTED_MEMORY;
