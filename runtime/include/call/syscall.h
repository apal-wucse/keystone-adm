...
setup_adm_syscall_metadata(size_t call_num, size_t argsize, size_t bufsize) {
  ...
  if (err == ADM_ERR_NOT_AVAILABLE) return (uintptr_t)NULL;
  // create metadata region
  // info + args + buffer + retval (uintptr_t)
  base = (uintptr_t)adm_new_region(
      sizeof(struct edge_syscall) + argsize + bufsize + sizeof(uintptr_t),
      ADM_UID_SYSCALL_META, TYPE_STATIC_ARR_BYTE);
  if (!base) return (uintptr_t)NULL;
