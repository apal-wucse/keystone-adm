...
extern pte paging_l2_page_table[BIT(RISCV_PT_INDEX_BITS)] __attribute__((aligned(RISCV_PAGE_SIZE)));
extern pte paging_l3_page_table[BIT(RISCV_PT_INDEX_BITS)] __attribute__((aligned(RISCV_PAGE_SIZE)));
...
uintptr_t paging_backing_region_size(void);
bool paging_epm_inbounds(uintptr_t addr);
bool paging_backpage_inbounds(uintptr_t addr);
