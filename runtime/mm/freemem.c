...
__spa_get(bool zero) {
  ...
  assert(
      free_page > EYRIE_LOAD_START &&
      free_page < (freemem_va_start + freemem_size));

  if (zero) memset((void*)free_page, 0, RISCV_PAGE_SIZE);
...
spa_init(uintptr_t base, size_t size) {
  ...
  for (cur = base; cur < base + size; cur += RISCV_PAGE_SIZE) {
    spa_put(cur);
  }
}
#endif  // USE_FREEMEM
