...
uintptr_t paging_alloc_backing_page() {
    uintptr_t offs_update = (paging_next_backing_page_offset + paging_inc_backing_page_offset_by) %
    ...
    uintptr_t next_page = paging_backing_region() + paging_next_backing_page_offset;
...
unsigned int paging_remaining_pages() {
    return (paging_backing_region_size() - paging_next_backing_page_offset) / RISCV_PAGE_SIZE;
}

static uintptr_t gcd(uintptr_t a, uintptr_t b) {
    while (b) {
        uintptr_t tmp = b;
        b             = a % b;
        a             = tmp;
...
static uintptr_t find_coprime_of(uintptr_t n) {
    uintptr_t res;
    do {
...
static uint64_t* pswap_pageout_ctr(uintptr_t page) {
...
static void pswap_establish_boot_key(void) {
...
static void pswap_encrypt(const void* addr, void* dst, uint64_t pageout_ctr) {
...
static void pswap_decrypt(const void* addr, void* dst, uint64_t pageout_ctr) {
    size_t len = RISCV_PAGE_SIZE;
...
static void pswap_hash(uint8_t* hash, void* page_addr, uint64_t pageout_ctr) {
#ifdef USE_PAGE_HASH
    SHA256_CTX hasher;
