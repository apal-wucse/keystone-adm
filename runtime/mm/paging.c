...
void paging_inc_user_page(void) { paging_user_page_count++; }
...
void init_paging(uintptr_t user_pa_start, uintptr_t user_pa_end) {
    ...
    pswap_init();
    debug(
        "BACK: 0x%lx-0x%lx (%u KB), va 0x%lx", addr, addr + size, size / 1024,
        paging_backing_storage_addr);

    /* create VA mapping, we don't give execution perm */
    map_with_reserved_page_table(
        addr, size, EYRIE_PAGING_START, paging_l2_page_table, paging_l3_page_table);
    /*
...
uintptr_t
__traverse_page_table_and_pick_internal(int level, pte* tb, uintptr_t vaddr, uintptr_t* count) {
    pte* walk;
    int i = 0;

    assert(count);

    for (walk = tb, i = 0; walk < tb + (RISCV_PAGE_SIZE / sizeof(pte)); walk += 1, i++) {
        if (*walk == 0)
            continue;
        ...
        /* if this is a leaf */
        if (level == 1 || (entry & PTE_R) || (entry & PTE_W) || (entry & PTE_X)) {
            ...
            ret = __traverse_page_table_and_pick_internal(
                level - 1, (pte*)__va(phys_addr), vpn(virt_addr), count);
            if (ret)
                return ret;
        }
    }
...
uintptr_t __traverse_page_table_and_pick(uintptr_t count) {
...
uintptr_t paging_evict_and_free_one(uintptr_t swap_va) {
    ...
    assert(dest_va < paging_backing_storage_addr + paging_backing_storage_size);
    ...
    *target_pte = pte_create_invalid(ppn(__paging_pa(dest_va)), *target_pte & PTE_FLAG_MASK);
...
bool paging_epm_inbounds(uintptr_t epm_page) {
    return (epm_page >= EYRIE_LOAD_START) && (epm_page < freemem_va_start + freemem_size);
}

bool paging_backpage_inbounds(uintptr_t back_page) {
    return (back_page >= paging_backing_storage_addr) &&
           (back_page < paging_backing_storage_addr + paging_backing_storage_size);
}

uintptr_t paging_backing_region(void) { return paging_backing_storage_addr; }

uintptr_t paging_backing_region_size(void) { return paging_backing_storage_size; }
