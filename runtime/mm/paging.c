...
void
paging_inc_user_page(void) {
  paging_user_page_count++;
}

void
paging_dec_user_page(void) {
  paging_user_page_count--;
  assert(paging_user_page_count >= 0);
}

void
init_paging(uintptr_t user_pa_start, uintptr_t user_pa_end) {
  uintptr_t addr        = 0;
  uintptr_t size        = 0;
  ...
  map_with_reserved_page_table(
      addr, size, EYRIE_PAGING_START, paging_l2_page_table,
      paging_l3_page_table);
  /*
...
__traverse_page_table_and_pick_internal(
    int level, pte* tb, uintptr_t vaddr, uintptr_t* count) {
  pte* walk;
  int i = 0;
  ...
  for (walk = tb, i = 0; walk < tb + (RISCV_PAGE_SIZE / sizeof(pte));
    ...
    uintptr_t virt_addr = ((vaddr << RISCV_PT_INDEX_BITS) | (i & 0x1ff))
                          << RISCV_PAGE_BITS;
    ...
    if (level == 1 || (entry & PTE_R) || (entry & PTE_W) || (entry & PTE_X)) {
      ...
      if (*count == 0) return virt_addr;
    } else {
      uintptr_t ret = 0;

      /* extending MSB */
      if (level == 3 && (i & 0x100)) vaddr = 0xffffffffffffffffUL;
...
paging_evict_and_free_one(uintptr_t swap_va) {
  ...
  *target_pte = pte_create_invalid(
      ppn(__paging_pa(dest_va)), *target_pte & PTE_FLAG_MASK);
...
void
paging_handle_page_fault(struct encl_ctx* ctx) {
  uintptr_t addr;
  uintptr_t back_ptr;
  uintptr_t frame;
  pte* entry;
  ...
  /* VA legitimacy check */
  if (addr >= EYRIE_LOAD_START) goto exit;
