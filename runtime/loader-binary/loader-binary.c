...
uintptr_t satp_new(uintptr_t pa) { return (SATP_MODE | (pa >> RISCV_PAGE_BITS)); }
...
void map_physical_memory(uintptr_t dram_base, uintptr_t dram_size) {
    ...
    map_with_reserved_page_table(
        dram_base, dram_size, ptr, load_l2_page_table_storage, load_l3_page_table_storage);
}
...
int map_additional_memory(uintptr_t additional_ptr, uintptr_t additional_size) {
...
int load_runtime(
    uintptr_t dummy, uintptr_t dram_base, uintptr_t dram_size, uintptr_t runtime_base,
    uintptr_t user_base, uintptr_t free_base, uintptr_t untrusted_ptr, uintptr_t untrusted_size,
    uintptr_t additional_ptr, uintptr_t additional_size) {
    int ret = 0;
