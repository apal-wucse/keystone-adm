...
static inline int
pt_mode_from_elf(int elf_pt_mode) {
  return (((elf_pt_mode & PF_X) > 0) * PTE_X) |
         (((elf_pt_mode & PF_W) > 0) * (PTE_W | PTE_R | PTE_D)) |
         (((elf_pt_mode & PF_R) > 0) * PTE_R);
}

int
loadElf(elf_t* elf, bool user) {
  for (unsigned int i = 0; i < elf_getNumProgramHeaders(elf); i++) {
    ...
    /* va is not page-aligned, so it doesn't own some of the page. Page may
     * already be mapped. */
    if (RISCV_PAGE_OFFSET(va)) {
      ...
      memcpy(
          (void*)(new_page + RISCV_PAGE_OFFSET(va)), src,
          RISCV_PAGE_SIZE - RISCV_PAGE_OFFSET(va));
    ...
    while (va + RISCV_PAGE_SIZE <= file_end) {
      ...
      if (!map_page(vpn(va), ppn(src_pa), pt_mode)) return -1;
      src += RISCV_PAGE_SIZE;
      va += RISCV_PAGE_SIZE;
    }
