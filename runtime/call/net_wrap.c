...
uintptr_t
io_syscall_socket(int domain, int type, int protocol) {
  uintptr_t ret = -1;
...
io_syscall_setsockopt(
  ...
  if (edge_call_check_ptr_valid((uintptr_t)&args->option_value, option_len) !=
      0) {
    goto done;
  }
  ...
  size_t totalsize =
      sizeof(struct edge_syscall) + sizeof(sargs_SYS_setsockopt) + option_len;
  ret = dispatch_edgecall_syscall(edge_syscall, totalsize);
...
uintptr_t
io_syscall_connect(int sockfd, uintptr_t addr, socklen_t addrlen) {
  uintptr_t ret = -1;
  struct edge_syscall* edge_syscall =
      (struct edge_syscall*)edge_call_data_ptr();
...
uintptr_t
io_syscall_bind(int sockfd, uintptr_t addr, socklen_t addrlen) {
  uintptr_t ret = -1;
  struct edge_syscall* edge_syscall =
      (struct edge_syscall*)edge_call_data_ptr();
...
uintptr_t
io_syscall_listen(int sockfd, int backlog) {
  uintptr_t ret = -1;
  struct edge_syscall* edge_syscall =
      (struct edge_syscall*)edge_call_data_ptr();
...
uintptr_t
io_syscall_accept(int sockfd, uintptr_t addr, uintptr_t addrlen) {
  uintptr_t ret = -1;
  struct edge_syscall* edge_syscall =
      (struct edge_syscall*)edge_call_data_ptr();
...
uintptr_t
io_syscall_recvfrom(
    int sockfd, uintptr_t buf, size_t len, int flags, uintptr_t src_addr,
    uintptr_t addrlen) {
  uintptr_t ret = -1;
  struct edge_syscall* edge_syscall =
      (struct edge_syscall*)edge_call_data_ptr();
  ...
  if (src_addr != 0) {
    ...
    if (edge_call_check_ptr_valid(
            (uintptr_t)&args->addrlen, sizeof(socklen_t)) != 0) {
      goto done;
    }
    copy_from_user(&args->addrlen, (void*)addrlen, sizeof(socklen_t));
    if (edge_call_check_ptr_valid((uintptr_t)&args->src_addr, args->addrlen) !=
        0) {
      goto done;
    }
...
uintptr_t
io_syscall_sendto(
    int sockfd, uintptr_t buf, size_t len, int flags, uintptr_t dest_addr,
    int addrlen) {
  uintptr_t ret = -1;
  struct edge_syscall* edge_syscall =
      (struct edge_syscall*)edge_call_data_ptr();
  ...
  if (dest_addr != 0) {
    ...
    args->addrlen           = addrlen;
    if (edge_call_check_ptr_valid((uintptr_t)&args->dest_addr, args->addrlen) !=
        0) {
      goto done;
    }
  ...
  size_t totalsize =
      sizeof(struct edge_syscall) + sizeof(sargs_SYS_sendto) + len;
  ret = dispatch_edgecall_syscall(edge_syscall, totalsize);
...
uintptr_t
io_syscall_sendfile(int out_fd, int in_fd, uintptr_t offset, int count) {
  uintptr_t ret = -1;
  struct edge_syscall* edge_syscall =
      (struct edge_syscall*)edge_call_data_ptr();
  ...
  if (offset != 0) {
    ...
    if (edge_call_check_ptr_valid((uintptr_t)&args->offset, sizeof(off_t)) !=
        0) {
      goto done;
    }
...
uintptr_t
io_syscall_getpeername(int sockfd, uintptr_t addr, uintptr_t addrlen) {
  uintptr_t ret = -1;
  struct edge_syscall* edge_syscall =
      (struct edge_syscall*)edge_call_data_ptr();
  ...
  size_t totalsize =
      (sizeof(struct edge_syscall)) + sizeof(sargs_SYS_getpeername);
  ret = dispatch_edgecall_syscall(edge_syscall, totalsize);

  copy_to_user(
      (void*)addr, &args->addr,
      args->addrlen > *(socklen_t*)addrlen ? *(socklen_t*)addrlen
                                           : args->addrlen);
  copy_to_user((void*)addrlen, &args->addrlen, sizeof(socklen_t));

  print_strace(
      "[runtime] proxied getpeername: fd: %d, ret: %d\r\n", args->sockfd, ret);
  return ret;
}

uintptr_t
io_syscall_getsockname(int sockfd, uintptr_t addr, uintptr_t addrlen) {
  uintptr_t ret = -1;
  struct edge_syscall* edge_syscall =
      (struct edge_syscall*)edge_call_data_ptr();
  ...
  size_t totalsize =
      (sizeof(struct edge_syscall)) + sizeof(sargs_SYS_getsockname);
  ret = dispatch_edgecall_syscall(edge_syscall, totalsize);
  ...
  print_strace(
      "[runtime] proxied getsockname: fd: %d, ret: %d\r\n", args->sockfd, ret);
  return ret;
}
...
uintptr_t
io_syscall_pselect(
    int nfds, uintptr_t readfds, uintptr_t writefds, uintptr_t exceptfds,
    uintptr_t timeout, uintptr_t sigmask) {
  uintptr_t ret = -1;
  struct edge_syscall* edge_syscall =
      (struct edge_syscall*)edge_call_data_ptr();
  ...
  print_strace(
      "[runtime] proxied pselect: nfds: %d, ret: %d\r\n", args->nfds, ret);
  return ret;
}
...
io_syscall_socket(int domain, int type, int protocol) {
  ...
  edge_syscall = (struct edge_syscall*)setup_adm_syscall_metadata(
      SYS_socket, sizeof(sargs_SYS_socket), 0);
...
io_syscall_setsockopt(
  ...
  if (offset) {
    ...
    edge_syscall = (struct edge_syscall*)setup_adm_syscall_metadata(
        SYS_setsockopt | SYSCALL_ADMPTR_BUFFER, sizeof(sargs_SYS_setsockopt_p),
        0);
    if (!edge_syscall) goto done;
...
io_syscall_connect(int sockfd, uintptr_t addr, socklen_t addrlen) {
  ...
  if (addrlen > sizeof(struct sockaddr_storage)) return -1;
...
io_syscall_bind(int sockfd, uintptr_t addr, socklen_t addrlen) {
  ...
  if (offset) {
    ...
    sargs_SYS_bind* args;
    edge_syscall = (struct edge_syscall*)setup_adm_syscall_metadata(
        SYS_bind, sizeof(sargs_SYS_bind), 0);
    if (!edge_syscall) goto done;
...
io_syscall_listen(int sockfd, int backlog) {
  uintptr_t ret = -1;
  struct edge_syscall* edge_syscall =
      (struct edge_syscall*)setup_adm_syscall_metadata(
          SYS_listen, sizeof(sargs_SYS_listen), 0);
  if (!edge_syscall) goto done;
...
io_syscall_accept(int sockfd, uintptr_t addr, uintptr_t addrlen) {
  ...
  if (addr_offset && addrlen_offset) {
    ...
    if (args->addrlen > sizeof(struct sockaddr_storage)) return ret;
...
io_syscall_recvfrom(
  ...
  if (src_addr != 0) {
    ...
    if (buf_offset && addr_offset && addrlen_offset) {
      ...
      edge_syscall = (struct edge_syscall*)setup_adm_syscall_metadata(
          SYS_recvfrom | SYSCALL_ADMPTR_BUFFER, sizeof(sargs_SYS_recvfrom_p),
          0);
      if (!edge_syscall) goto done;
      ...
      if (ret > 0) copy_to_user((void*)buf, &args->buf, ret > len ? len : ret);
    }
  } else {
    uintptr_t buf_offset = __adm_offset(buf);
    if (buf_offset) {
...
io_syscall_getpeername(int sockfd, uintptr_t addr, uintptr_t addrlen) {
  ...
  if (addr_off && addrlen_off) {
    ...
    edge_syscall = (struct edge_syscall*)setup_adm_syscall_metadata(
        SYS_getpeername | SYSCALL_ADMPTR_BUFFER,
        sizeof(sargs_SYS_getpeername_p), 0);
    if (!edge_syscall) goto done;
...
io_syscall_getsockname(int sockfd, uintptr_t addr, uintptr_t addrlen) {
  ...
  if (addr_off && addrlen_off) {
    ...
    edge_syscall = (struct edge_syscall*)setup_adm_syscall_metadata(
        SYS_getsockname | SYSCALL_ADMPTR_BUFFER,
        sizeof(sargs_SYS_getsockname_p), 0);
    if (!edge_syscall) goto done;
...
io_syscall_pselect(
  ...
  bool is_no_copy = (!readfds || readfds_off) && (!writefds || writefds_off) &&
                    (!exceptfds || exceptfds_off) &&
                    (!timeout || timeout_off) && (!sigmask || sigmask_off);
  bool is_with_copy =
      !((readfds && readfds_off) || (writefds && writefds_off) ||
