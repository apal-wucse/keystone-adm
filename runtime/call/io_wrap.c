...
uintptr_t io_syscall_ftruncate(int fd, off_t offset) {
    ...
    size_t totalsize = (sizeof(struct edge_syscall) + sizeof(sargs_SYS_ftruncate));
...
uintptr_t io_syscall_read(int fd, void* buf, size_t len) {
    ...
    size_t totalsize = (sizeof(struct edge_syscall) + sizeof(sargs_SYS_read) + len);
...
uintptr_t io_syscall_write(int fd, void* buf, size_t len) {
    ...
    size_t totalsize = (sizeof(struct edge_syscall) + sizeof(sargs_SYS_write) + len);
...
uintptr_t io_syscall_openat(int dirfd, char* path, int flags, mode_t mode) {
    struct edge_syscall* edge_syscall = (struct edge_syscall*)edge_call_data_ptr();
    ...
    size_t totalsize = (sizeof(struct edge_syscall) + sizeof(sargs_SYS_openat) + pathlen);
...
uintptr_t io_syscall_unlinkat(int dirfd, char* path, int flags) {
    struct edge_syscall* edge_syscall = (struct edge_syscall*)edge_call_data_ptr();
    ...
    size_t totalsize = (sizeof(struct edge_syscall) + sizeof(sargs_SYS_unlinkat) + pathlen);
...
uintptr_t io_syscall_fstatat(int dirfd, char* pathname, struct stat* statbuf, int flags) {
    struct edge_syscall* edge_syscall = (struct edge_syscall*)edge_call_data_ptr();
    ...
    size_t totalsize = (sizeof(struct edge_syscall) + sizeof(sargs_SYS_fstatat) + pathlen);
...
uintptr_t io_syscall_fcntl(int fd, int cmd, uintptr_t arg) {
    ...
    if (cmd == F_SETLK || cmd == F_SETLKW || cmd == F_GETLK) {
        ...
        totalsize = (sizeof(struct edge_syscall) + sizeof(sargs_SYS_fcntl) + sizeof(struct flock));
    } else {
        args->arg[0]     = arg;
        args->has_struct = 0;
...
uintptr_t io_syscall_renameat2(
    int olddirfd, uintptr_t oldpath, int newdirfd, uintptr_t newpath, unsigned int flags) {
    uintptr_t ret                     = -1;
    struct edge_syscall* edge_syscall = (struct edge_syscall*)edge_call_data_ptr();
...
uintptr_t io_syscall_sync() {
    ...
    edge_syscall = (struct edge_syscall*)setup_adm_syscall_metadata(SYS_sync, 0, 0);
    if (!edge_syscall)
        return -1;

    ret = dispatch_edgecall_syscall_protected(edge_syscall, sizeof(struct edge_syscall), false);
    print_strace("[runtime] proxied sync\r\n");
    return ret;
}
...
uintptr_t io_syscall_fsync(int fd) {
    ...
    edge_syscall =
        (struct edge_syscall*)setup_adm_syscall_metadata(SYS_fsync, sizeof(sargs_SYS_fsync), 0);
    if (!edge_syscall)
        return -1;
...
uintptr_t io_syscall_lseek(int fd, off_t offset, int whence) {
    ...
    edge_syscall =
        (struct edge_syscall*)setup_adm_syscall_metadata(SYS_lseek, sizeof(sargs_SYS_lseek), 0);
    if (!edge_syscall)
        return -1;
...
uintptr_t io_syscall_close(int fd) {
    ...
    edge_syscall =
        (struct edge_syscall*)setup_adm_syscall_metadata(SYS_close, sizeof(sargs_SYS_close), 0);
    if (!edge_syscall)
        return -1;
...
uintptr_t io_syscall_read(int fd, void* buf, size_t len) {
    ...
    if (buf_offset) { // no copy
        ...
        sargs_SYS_read* args;
        edge_syscall =
            (struct edge_syscall*)setup_adm_syscall_metadata(SYS_read, sizeof(sargs_SYS_read), len);
        if (!edge_syscall)
            return -1;
...
uintptr_t io_syscall_openat(int dirfd, char* path, int flags, mode_t mode) {
    ...
    if (buf_offset) {
        ...
        edge_syscall = (struct edge_syscall*)setup_adm_syscall_metadata(
            SYS_openat | SYSCALL_ADMPTR_BUFFER, sizeof(sargs_SYS_openat_p), pathlen);
        if (!edge_syscall)
            goto done;
...
uintptr_t io_syscall_unlinkat(int dirfd, char* path, int flags) {
    ...
    if (buf_offset) {
        ...
        data_len = sizeof(struct edge_syscall) + sizeof(sargs_SYS_unlinkat) + pathlen;
    }
...
uintptr_t io_syscall_fstatat(int dirfd, char* pathname, struct stat* statbuf, int flags) {
    ...
    if (pathbuf) {
        ...
        data_len = sizeof(struct edge_syscall) + sizeof(sargs_SYS_fstatat) + pathlen;
        ret      = dispatch_edgecall_syscall_protected(edge_syscall, data_len, true);
        if (ret == 0) {
            copy_to_user(statbuf, &args->stats, sizeof(struct stat));
        }
...
uintptr_t io_syscall_pipe(int* fds) { // int fds[2]
    uintptr_t ret = -1;
    struct edge_syscall* edge_syscall;
    size_t data_len;

    edge_syscall = (struct edge_syscall*)setup_adm_syscall_metadata(SYS_pipe2, sizeof(int) * 2, 0);
    if (!edge_syscall)
        goto done;
...
uintptr_t io_syscall_epoll_ctl(int epfd, int op, int fd, uintptr_t event) {
    ...
    if (event) {
        ...
        data_len = sizeof(struct edge_syscall) + sizeof(struct sargs_SYS_epoll_ctl_p);
        ret      = dispatch_edgecall_syscall_protected(edge_syscall, data_len, true);
    } else {
...
uintptr_t io_syscall_fcntl(int fd, int cmd, uintptr_t arg) {
    ...
    if (cmd == F_SETLK || cmd == F_SETLKW || cmd == F_GETLK) {
        ...
        if (struct_offset) {
            ...
            data_len = sizeof(struct edge_syscall) + sizeof(sargs_SYS_fcntl) + sizeof(struct flock);
        }
    } else {
        sargs_SYS_fcntl* args;
        edge_syscall = (struct edge_syscall*)setup_adm_syscall_metadata(
        ...
        *(uintptr_t*)args->arg = arg;
        ...
        data_len = sizeof(struct edge_syscall) + sizeof(sargs_SYS_fcntl) + sizeof(uintptr_t);
    }
...
uintptr_t io_syscall_epoll_pwait(int epfd, uintptr_t events, int maxevents, int timeout) {
    ...
    if (offset) {
        ...
        edge_syscall = (struct edge_syscall*)setup_adm_syscall_metadata(
            SYS_epoll_pwait | SYSCALL_ADMPTR_BUFFER, sizeof(sargs_SYS_epoll_pwait_p), 0);
        if (!edge_syscall)
            goto done;
...
uintptr_t io_syscall_renameat2(
    ...
    if (oldpath_offset && newpath_offset) {
        ...
        edge_syscall = (struct edge_syscall*)setup_adm_syscall_metadata(
            SYS_renameat2 | SYSCALL_ADMPTR_BUFFER, sizeof(sargs_SYS_renameat2_p), 0);
        if (!edge_syscall)
            goto done;
...
uintptr_t io_syscall_umask(int mask) {
    ...
    edge_syscall =
        (struct edge_syscall*)setup_adm_syscall_metadata(SYS_umask, sizeof(sargs_SYS_umask), 0);
    if (!edge_syscall)
        return (uintptr_t)-1;
...
uintptr_t io_syscall_fstat(int fd, struct stat* statbuf) {
    ...
    if (offset) {
        ...
        edge_syscall =
            (struct edge_syscall*)setup_adm_syscall_metadata(SYS_fstat, sizeof(sargs_SYS_fstat), 0);
        if (!edge_syscall)
            goto done;
