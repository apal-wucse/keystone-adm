...
int
verify_and_load_elf_file(uintptr_t ptr, size_t file_size, bool is_eapp) {
  int ret = 0;
  // validate elf
...
init_user_stack_and_env(ELF(Ehdr) * hdr) {
  ...
  count = alloc_pages(
      vpn(stack_end), stack_count, PTE_R | PTE_W | PTE_D | PTE_A | PTE_U);
...
eyrie_boot(
    ...
    uintptr_t dram_base, uintptr_t dram_size, uintptr_t runtime_paddr,
    uintptr_t user_paddr, uintptr_t free_paddr, uintptr_t utm_paddr,
    uintptr_t utm_size,
    uintptr_t adm_paddr,  // assumed to be stored in sp+0
    uintptr_t adm_size)   // assumed to be stored in sp+8
{
  /* set initial values */
  ...
  additional_memory      = EYRIE_ADDITIONAL_START;
  additional_memory_size = adm_size;
  additional_pa          = adm_paddr;
  additional_user_va     = 0;
  runtime_va_start       = (uintptr_t)&rt_base;
  kernel_offset          = runtime_va_start - runtime_paddr;

  debug("ROOT PAGE TABLE: 0x%lx", root_page_table);
  debug(
      "UTM : 0x%lx-0x%lx (%u KB)", utm_paddr, utm_paddr + utm_size,
      utm_size / 1024);
  debug(
      "ADM : 0x%lx-0x%lx (%u KB)", adm_paddr, adm_paddr + adm_size,
      adm_size / 1024);
  debug(
      "DRAM: 0x%lx-0x%lx (%u KB)", dram_base, dram_base + dram_size,
      dram_size / 1024);
  debug(
      "USER: 0x%lx-0x%lx (%u KB)", user_paddr, free_paddr,
      (free_paddr - user_paddr) / 1024);
  ...
  debug(
      "FREE: 0x%lx-0x%lx (%u KB), va 0x%lx", free_paddr, dram_base + dram_size,
      freemem_size / 1024, freemem_va_start);

  /* initialize free memory */
  init_freemem();

  /* load eapp elf */
  assert(!verify_and_load_elf_file(
      __va(user_paddr), free_paddr - user_paddr, true));

  /* free leaking memory */
  // TODO: clean up after loader -- entire file no longer needed
  // TODO: load elf file doesn't map some pages; those can be re-used. runtime
  // and eapp.
  ...
  /* prepare additional memory */
  init_adm_internals(adm_paddr);
