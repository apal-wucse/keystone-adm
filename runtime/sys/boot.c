...
void init_freemem() { spa_init(freemem_va_start, freemem_size); }

#endif // USE_FREEMEM

/* initialize user stack */
void init_user_stack_and_env(ELF(Ehdr) * hdr) {
    ...
    count = alloc_pages(vpn(stack_end), stack_count, PTE_R | PTE_W | PTE_D | PTE_A | PTE_U);
...
void eyrie_boot(
    ...
    uintptr_t dram_base, uintptr_t dram_size, uintptr_t runtime_paddr, uintptr_t user_paddr,
    uintptr_t free_paddr, uintptr_t utm_paddr, uintptr_t utm_size,
    ...
    uintptr_t adm_size)  // assumed to be stored in sp+8
{
    ...
    debug("ADM : 0x%lx-0x%lx (%u KB)", adm_paddr, adm_paddr + adm_size, adm_size / 1024);
