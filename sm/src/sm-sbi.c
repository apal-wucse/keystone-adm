...
unsigned long sbi_sm_stop_enclave_with_share(
    ...
    unsigned long ret;
    if (type_info) {
        ret = copy_adm_type_info(type_info, count, type_info_local);
        if (ret)
            // Copy failed
            regs->a0 = ret;
        else
            regs->a0 = stop_enclave_with_share(
                regs, cpu_get_enclave_id(), type_info_local, count, share_type);
    } else {
        regs->a0 = stop_enclave_with_share(regs, cpu_get_enclave_id(), 0, 0, share_type);
    }
    regs->mepc += 4;
    sbi_trap_exit(regs);
    return 0;
...
unsigned long
sbi_sm_get_sealing_key(uintptr_t sealing_key, uintptr_t key_ident, size_t key_ident_size) {
    unsigned long ret;
    ret = get_sealing_key(sealing_key, key_ident, key_ident_size, cpu_get_enclave_id());
    return ret;
}

unsigned long sbi_sm_random() { return (unsigned long)platform_random(); }
