...
unsigned long platform_init_global_once() {
    ...
    if (pmp_region_init_atomic(
            CACHE_CONTROLLER_ADDR_START, CACHE_CONTROLLER_ADDR_END - CACHE_CONTROLLER_ADDR_START,
    ...
    if (pmp_region_init_atomic(
            L2_SCRATCH_START, L2_SCRATCH_STOP - L2_SCRATCH_START, PMP_PRI_ANY, &scratch_rid, 1)) {
...
unsigned long platform_create_enclave(struct enclave* enclave) {
    ...
    if (enclave->ped.use_scratch) {
        ...
        enclave->pa_params.runtime_base =
            (scratch_epm_start + (enclave->pa_params.runtime_base - old_epm_start));
        enclave->pa_params.user_base =
            (scratch_epm_start + (enclave->pa_params.user_base - old_epm_start));
        enclave->pa_params.free_base = (scratch_epm_start + size);
        enclave->encl_satp =
            ((scratch_epm_start >> RISCV_PGSHIFT) | (SATP_MODE_SV39 << HGATP_MODE_SHIFT));
...
void platform_switch_to_enclave(struct enclave* enclave) {
    ...
    if (enclave->ped.num_ways > 0) {
        ...
        waymask_allocate_ways(enclave->ped.num_ways, core, &enclave->ped.saved_mask);
