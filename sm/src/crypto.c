...
void hash_init(hash_ctx* hash_ctx) { sha3_init(hash_ctx, MDSIZE); }
...
void hash_finalize(void* md, hash_ctx* hash_ctx) { sha3_final(md, hash_ctx); }

void sign(
    void* sign, const void* data, size_t len, const unsigned char* public_key,
    const unsigned char* private_key) {
    ed25519_sign(sign, data, len, public_key, private_key);
}

int
kdf(const unsigned char* salt, size_t salt_len, const unsigned char* ikm, size_t ikm_len,
