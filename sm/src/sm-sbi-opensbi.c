...
static int sbi_ecall_keystone_enclave_handler(
    unsigned long extid, unsigned long funcid, const struct sbi_trap_regs *regs,
    unsigned long *out_val, struct sbi_trap_info *out_trap) {
    uintptr_t retval;
    ...
    case SBI_SM_EXIT_ENCLAVE:
        ...
        __builtin_unreachable();
        break;
    case SBI_SM_STOP_ENCLAVE_SHARE:
        retval = sbi_sm_stop_enclave_with_share(
            (struct sbi_trap_regs *)regs, regs->a0, regs->a1, regs->a2);
        __builtin_unreachable();
        break;
