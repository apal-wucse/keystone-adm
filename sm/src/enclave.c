...
context_switch_to_enclave(struct sbi_trap_regs *regs, enclave_id eid, int load_parameters) {
    ...
    if (load_parameters) {
        ...
        regs->mepc = (uintptr_t)enclaves[eid].pa_params.dram_base -
                     4; // regs->mepc will be +4 before sbi_ecall_handler return
        ...
        // $t1: (PA) adm base
        regs->t1 = (uintptr_t)enclaves[eid].params.additional_ptr;
        // $t2: (size_t) adm size
        regs->t2 = (uintptr_t)enclaves[eid].params.additional_size;
    ...
    for (memid = 0; memid < ENCLAVE_REGIONS_MAX; memid++) {
        if (enclaves[eid].regions[memid].type == REGION_ADM) {
            pmp_set_keystone(enclaves[eid].regions[memid].pmp_rid, PMP_RW_PERM);
        ...
        if (enclaves[eid].regions[memid].type == REGION_ADM) {
            ...
            if (default_perm == DEFAULT_READ) {
                if (share_type == ADM_SHARE_RW) {
                    // read/write share
                    pmp_set_global(enclaves[eid].regions[memid].pmp_rid, PMP_RW_PERM);
                } else if (share_type == ADM_SHARE_RONLY) {
                    // assume another harts already have read-only permission. remove write
                    // permission from only enclave hart.
                    pmp_set_keystone(enclaves[eid].regions[memid].pmp_rid, PMP_R);
                } else { // ADM_SHARE_NONE or invalid type
                    // fallback to default
                    enclaves[eid].adm_state_info.share = ADM_SHARE_RONLY;
                    pmp_set_keystone(enclaves[eid].regions[memid].pmp_rid, PMP_R);
                }
            } else if (default_perm == DEFAULT_NOPERM) {
                if (share_type == ADM_SHARE_RW) {
                    // read/write share
                    pmp_set_global(enclaves[eid].regions[memid].pmp_rid, PMP_RW_PERM);
                } else if (share_type == ADM_SHARE_RONLY) {
                    // read-only share
                    pmp_set_global(enclaves[eid].regions[memid].pmp_rid, PMP_R);
                } else if (share_type == ADM_SHARE_NONE) {
                    // assume another harts have no permission. remove all permission from only
                    // enclave hart.
                ...
                enclaves[eid].adm_state_info.share = ADM_SHARE_NONE;
                pmp_set_global(enclaves[eid].regions[memid].pmp_rid, PMP_NO_PERM);
            }
        } else if (enclaves[eid].regions[memid].type != REGION_INVALID) {
            pmp_set_keystone(enclaves[eid].regions[memid].pmp_rid, PMP_NO_PERM);
        }
...
void enclave_init_metadata() {
    ...
    for (eid = 0; eid < ENCL_MAX; eid++) {
        ...
        // Initialize ADM validation info
        enclaves[eid].adm_default_protection = DEFAULT_NOPERM;
        enclaves[eid].adm_state_info.share =
            ADM_SHARE_RW; // validation is hooked when ADM_SHARE_RW is set.
...
static int is_create_args_valid(struct keystone_sbi_create *args) {
    ...
    if (args->utm_region.paddr >= args->utm_region.paddr + args->utm_region.size)
        return 0;
    if (args->adm_region.paddr != 0 && args->adm_region.size != 0) {
        if (args->adm_region.paddr >= args->adm_region.paddr + args->adm_region.size)
            return 0;
    }
...
unsigned long create_enclave(unsigned long *eidptr, struct keystone_sbi_create create_args) {
    ...
    enclave_id eid;
    unsigned long ret;
    int region, shared_region, adm_region;
    ...
    // create a PMP region bound to the additional data memory
    if (adm_enabled && pmp_region_init_atomic(admbase, admsize, PMP_PRI_ANY, &adm_region, 0))
        goto free_region;

    // create PMP region for shared memory
    if (pmp_region_init_atomic(utbase, utsize, PMP_PRI_BOTTOM, &shared_region, 0))
        goto free_adm_region;
    ...
    if (adm_enabled) {
        switch (create_args.adm_protection) {
        case DEFAULT_READ: // default readable
            if (pmp_set_global(adm_region, PMP_R))
                goto unset_region;
            break;
        case DEFAULT_NOPERM: // default no permission
            if (pmp_set_global(adm_region, PMP_NO_PERM))
                goto unset_region;
            break;
    ...
    /* Initialize ADM information */
    if (adm_enabled) {
        enclaves[eid].adm_state_info.share = ADM_SHARE_RW; // enable validation on first run
        for (int i = 0; i < ADM_SLOT_MAX; i++)
            enclaves[eid].adm_state_info.type_info[i] =
                create_args.type_info[i]; // initial type info
    }
...
free_adm_region:
    if (adm_enabled) {
        pmp_region_free_atomic(adm_region);
    }
free_region:
...
unsigned long run_enclave(struct sbi_trap_regs *regs, enclave_id eid) {
    ...
    if (adm_memid >= 0) {
        if (share_type == ADM_SHARE_RW) { // ADM enabled and validation enabled
            ...
            for (int i = 0; i < ADM_SLOT_MAX; i++)
                enclaves[eid].adm_state_info.type_info[i] =
                    (AdmTypeInfo){ADM_UID_UNUSED, TYPE_NONE, 0};
        } else { // share state must be ADM_SHARE_RW before running
            return SBI_ERR_SM_ENCLAVE_NOT_FRESH;
        }
    }
...
unsigned long stop_enclave(struct sbi_trap_regs *regs, uint64_t request, enclave_id eid) {
    ...
    default_perm = enclaves[eid].adm_default_protection;
    if (default_perm == DEFAULT_NOPERM) {
        enclaves[eid].adm_state_info.share = ADM_SHARE_NONE;
    } else if (default_perm == DEFAULT_READ) {
        enclaves[eid].adm_state_info.share = ADM_SHARE_RONLY;
    } else {
        spin_unlock(&encl_lock);
        return SBI_ERR_SM_ENCLAVE_ADM_ILLEGAL_DATA;
...
unsigned long stop_enclave_with_share(
    ...
    enclaves[eid].adm_state_info.share = share_type;

    // set assumed data types
    if (type_info) {
        // Assumes invalidated all entries by default
        // Only copying used entries here
        for (int i = 0; i < count; i++)
            enclaves[eid].adm_state_info.type_info[i] = type_info[i];
    } else {
        // invalid state
        // r/w share w/o validation info
        if (share_type == ADM_SHARE_RW) {
            enclaves[eid].adm_state_info.share = ADM_SHARE_NONE;
            spin_unlock(&encl_lock);
            return SBI_ERR_SM_ENCLAVE_ILLEGAL_ARGUMENT;
...
unsigned long resume_enclave(struct sbi_trap_regs *regs, enclave_id eid) {
    ...
    if (adm_memid >= 0) {
        ...
        if (default_perm == DEFAULT_READ) {
            if (share_type == ADM_SHARE_RW) {
                int rid = enclaves[eid].regions[adm_memid].pmp_rid;
                pmp_set_global(rid, PMP_R);
            }
        } else if (default_perm == DEFAULT_NOPERM) {
            if (share_type == ADM_SHARE_RW || share_type == ADM_SHARE_RONLY) {
                int rid = enclaves[eid].regions[adm_memid].pmp_rid;
...
unsigned long attest_enclave(uintptr_t report_ptr, uintptr_t data, uintptr_t size, enclave_id eid) {
    ...
    spin_unlock(&encl_lock); // Don't need to wait while signing, which might
                             // take some time
    ...
    sm_sign(
        report.enclave.signature, &report.enclave,
        sizeof(struct enclave_report) - SIGNATURE_SIZE - ATTEST_DATA_MAXLEN + size);

    spin_lock(&encl_lock);

    /* copy report to the enclave */
    ret = copy_enclave_report(&enclaves[eid], report_ptr, &report);
