...
void swap_prev_mstatus(
    struct thread_state* thread, struct sbi_trap_regs* regs, uintptr_t current_mstatus) {
    // Time interrupts can occur in either user mode or supervisor mode
    uintptr_t mstatus_mask = MSTATUS_SIE | MSTATUS_SPIE | MSTATUS_SPP | MSTATUS_MPP | MSTATUS_FS |
...
void
swap_prev_state(struct thread_state* thread, struct sbi_trap_regs* regs, int return_on_resume) {
    int i;
...
void
swap_prev_mepc(struct thread_state* thread, struct sbi_trap_regs* regs, uintptr_t current_mepc) {
