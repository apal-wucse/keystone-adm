...
static inline int region_register_idx(region_id i) { return regions[i].reg_idx; }

static inline int region_allows_overlap(region_id i) { return regions[i].allow_overlap; }

static inline uintptr_t region_get_addr(region_id i) { return regions[i].addr; }

static inline uint64_t region_get_size(region_id i) { return regions[i].size; }

static inline int region_is_napot(region_id i) { return regions[i].addrmode == PMP_A_NAPOT; }

static inline int region_is_tor(region_id i) { return regions[i].addrmode == PMP_A_TOR; }
...
static void region_init(
    region_id i, uintptr_t addr, uint64_t size, uint8_t addrmode, int allow_overlap,
    pmpreg_id reg_idx) {
    regions[i].addr = addr;
    regions[i].size = size;
...
static pmpreg_id get_free_reg_idx() { return search_rightmost_unset(reg_bitmap, PMP_N_REG, 0x1); }
...
static int detect_region_overlap(uintptr_t addr, uintptr_t size) {
    ...
    for (i = 0; i < PMP_MAX_N_REGION; i++) {
        ...
        region_overlap |=
            ((uintptr_t)epm_base < input_end) && ((uintptr_t)epm_base + epm_size > addr);
    }
...
int pmp_set_keystone(int region_idx, uint8_t perm) {
    ...
    //        region_get_addr(region_idx), region_get_addr(region_idx) +
    //        region_get_size(region_idx), perm);
...
int pmp_region_init_atomic(
    uintptr_t start, uint64_t size, enum pmp_priority priority, region_id *rid, int allow_overlap) {
    int ret;
    spin_lock(&pmp_lock);
...
static int tor_region_init(
    ...
    *rid = region_idx;
    ...
    case (PMP_PRI_ANY): {
        ...
        if (TEST_BIT(reg_bitmap, reg_idx) || TEST_BIT(reg_bitmap, reg_idx + 1) ||
            reg_idx + 1 >= PMP_N_REG)
...
int pmp_region_init(
    uintptr_t start, uint64_t size, enum pmp_priority priority, int *rid, int allow_overlap) {
    if (!size)
    ...
    if ((size == -1UL && start == 0) || (!(size & (size - 1)) && !(start & (size - 1)))) {
        return napot_region_init(start, size, priority, rid, allow_overlap);
    } else {
        if (priority != PMP_PRI_ANY && (priority != PMP_PRI_TOP || start != 0)) {
            PMP_ERROR(
                SBI_ERR_SM_PMP_REGION_IMPOSSIBLE_TOR,
                "The top-priority TOR PMP entry must start from address 0");
        }
