...
static int fu540_console_init(void) {
    ...
    return sifive_uart_init(FU540_UART0_ADDR, peri_in_freq, FU540_UART_BAUDRATE);
}
...
static int fu540_irqchip_init(bool cold_boot) {
    ...
    return plic_warm_irqchip_init(
        &plic, (hartid) ? (2 * hartid - 1) : 0, (hartid) ? (2 * hartid) : -1);
}
...
static u64 fu540_get_tlbr_flush_limit(void) { return FU540_TLB_RANGE_FLUSH_LIMIT; }
