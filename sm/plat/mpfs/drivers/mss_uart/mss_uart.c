...
#define STATUS_ERROR_MASK                                                                          \
    (MSS_UART_OVERUN_ERROR | MSS_UART_PARITY_ERROR | MSS_UART_FRAMING_ERROR |                      \
     MSS_UART_BREAK_ERROR | MSS_UART_FIFO_ERROR)
...
int8_t MSS_UART_tx_complete(mss_uart_instance_t *this_uart) {
    ...
    if ((TX_COMPLETE == this_uart->tx_buff_size) && ((status & MSS_UART_TEMT) != 0u)) {
        ret_value = (int8_t)1;
    }
