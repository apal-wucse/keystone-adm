...
static inline int da9063_stop_watchdog(struct i2c_adapter* adap, uint32_t reg) {
    uint8_t val;
    int rc = i2c_adapter_reg_write(adap, reg, DA9063_REG_PAGE_CON, 0x00);
...
static inline int da9063_shutdown(struct i2c_adapter* adap, uint32_t reg) {
    ...
    return i2c_adapter_reg_write(adap, reg, DA9063_REG_CONTROL_F, DA9063_CONTROL_F_SHUTDOWN);
}
...
static inline int da9063_reset(struct i2c_adapter* adap, uint32_t reg) {
    ...
    rc = i2c_adapter_reg_write(adap, reg, DA9063_REG_CONTROL_F, DA9063_CONTROL_F_WAKEUP);
    if (rc)
        return rc;

    return i2c_adapter_reg_write(
        adap, reg, DA9063_REG_CONTROL_A,
        DA9063_CONTROL_A_M_POWER1_EN | DA9063_CONTROL_A_M_POWER_EN | DA9063_CONTROL_A_STANDBY);
}
...
static int da9063_reset_init(void* fdt, int nodeoff, const struct fdt_match* match) {
    int rc, i2c_bus;
    struct i2c_adapter* adapter;
    uint64_t addr;
...
static int sifive_fu740_final_init(bool cold_boot, const struct fdt_match* match) {
    int rc;
