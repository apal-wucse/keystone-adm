...
void Report::fromBytes(byte* bin) { std::memcpy(&report, bin, sizeof(struct report_t)); }
...
void Report::printJson() { std::cout << stringfy() << std::endl; }
...
void Report::printPretty() {
    ...
    std::cout << "Enclave Data: " << BytesToHex(report.enclave.data, report.enclave.data_len)
              << std::endl;
    std::cout << "\t\t-- Device pubkey --" << std::endl;
    std::cout << BytesToHex(report.dev_public_key, PUBLIC_KEY_SIZE) << std::endl;
}

byte* Report::getEnclaveHash() { return report.enclave.hash; }

byte* Report::getSmHash() { return report.sm.hash; }

int Report::verify(
    const byte* expected_enclave_hash, const byte* expected_sm_hash, const byte* dev_public_key) {
...
int Report::checkSignaturesOnly(const byte* dev_public_key) {
    ...
    sm_valid = ed25519_verify(
        report.sm.signature, reinterpret_cast<byte*>(&report.sm), MDSIZE + PUBLIC_KEY_SIZE,
        dev_public_key);
...
void* Report::getDataSection() { return report.enclave.data; }

size_t Report::getDataSize() { return report.enclave.data_len; }
