...
int edge_call_get_ptr_from_offset(edge_data_offset offset, size_t data_len, uintptr_t* ptr) {
    ...
    if (data_len > UINTPTR_MAX - (_shared_start + offset) || data_len > _shared_len - offset) {
        return -1;
    }
...
int edge_call_get_offset_from_ptr(uintptr_t ptr, size_t data_len, edge_data_offset* offset) {
...
int edge_call_ret_ptr(struct edge_call* edge_call, uintptr_t* ptr, size_t* size) {
    *size = edge_call->return_data.call_ret_size;
    return edge_call_get_ptr_from_offset(edge_call->return_data.call_ret_offset, *size, ptr);
}

int edge_call_setup_call(struct edge_call* edge_call, void* ptr, size_t size) {
    edge_call->call_arg_size = size;
    return edge_call_get_offset_from_ptr((uintptr_t)ptr, size, &edge_call->call_arg_offset);
}
...
int edge_call_setup_wrapped_ret(struct edge_call* edge_call, void* ptr, size_t size) {
    ...
    memcpy((void*)(_shared_start + sizeof(struct edge_call) + sizeof(struct edge_data)), ptr, size);

    memcpy(
        (void*)(_shared_start + sizeof(struct edge_call)), &data_wrapper, sizeof(struct edge_data));
...
uintptr_t edge_call_data_ptr() { return _shared_start + sizeof(struct edge_call); }
