...
adm_get_region(uintptr_t uid, uintptr_t* ptr, size_t* size) {
  ...
  if (!enabled || !_adm_hdr) return -1;
  for (size_t i = 0; i < _adm_hdr->data_n; i++) {
    if (_adm_hdr->uid_tbl[i] != uid) continue;  // skip unmatched uid
    region_hdr = adm_get_region_header(i);
    if (!region_hdr) return ADM_ERR_OUT_OF_BOUNDS;
    if (ptr != NULL) *ptr = (uintptr_t)adm_get_region_base(region_hdr);
    if (size != NULL) *size = region_hdr->size;
...
static uintptr_t
adm_setup_new_hdr(size_t size, uintptr_t uid, AdmDataTypes type) {
  ...
  /* check uid */
  for (size_t i = 0; i < _adm_hdr->data_n; i++) {
    if (_adm_hdr->uid_tbl[i] == uid) return (uintptr_t)NULL;
  }
  ...
  if (new_offset > UINTPTR_MAX - _adm_start || new_offset >= _adm_len)
    return (uintptr_t)NULL;
  if (size >
          UINTPTR_MAX - (_adm_start + new_offset + sizeof(AdmRegionHeader)) ||
  ...
  /* next offset */
  _adm_hdr->free_list =
      ((region + size) / DATA_ALIGN_BYTES + 1) * DATA_ALIGN_BYTES;
...
adm_remove_region(uintptr_t uid) {
  /* check validity */
  if (!enabled || !_adm_hdr) return ADM_ERR_NOT_AVAILABLE;

  for (size_t i = 0; i < _adm_hdr->data_n; i++) {
    /* look for specified uid */
    if (_adm_hdr->uid_tbl[i] != uid) continue;

    if (i < _adm_hdr->data_n - 1) {  // number of successor >= 1
      uintptr_t offset_gap =
          _adm_hdr->data_offsets[i + 1] - _adm_hdr->data_offsets[i];
...
adm_dynalloc_init(size_t size) {
  ...
  if (ptr && size != _size) return ADM_ERR_NOSPACE;

  if (!ptr) {
    ptr = (uintptr_t)adm_new_region(
        size, ADM_UID_MALLOC_SPACE, TYPE_DYN_ALLOC_SPACE);
    if (!ptr) return ADM_ERR_NOSPACE;
  }
...
adm_region_exists(uintptr_t uid) {
  for (size_t i = 0; i < _adm_hdr->data_n; i++) {
    if (_adm_hdr->uid_tbl[i] == uid) return true;
  }
  return false;
}
