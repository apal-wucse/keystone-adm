...
void incoming_syscall(struct edge_call* edge_call) {
    ...
    case (SYS_openat):;
        ...
        ret = openat(openat_args->dirfd, openat_args->path, openat_args->flags, openat_args->mode);
        break;
    case (SYS_unlinkat):;
        sargs_SYS_unlinkat* unlinkat_args = (sargs_SYS_unlinkat*)syscall_info->data;
        ret = unlinkat(unlinkat_args->dirfd, unlinkat_args->path, unlinkat_args->flags);
        break;
    case (SYS_ftruncate):;
    ...
    case (SYS_fstatat):;
        ...
        ret = fstatat(
            fstatat_args->dirfd, fstatat_args->pathname, &fstatat_args->stats, fstatat_args->flags);
        break;
    case (SYS_fstat):;
    ...
    case (SYS_epoll_pwait):;
        ...
        ret                                     = epoll_wait(
            epoll_pwait_args->epfd, &epoll_pwait_args->events, epoll_pwait_args->maxevents,
    ...
    case (SYS_renameat2):;
        ...
        ret                                = renameat(
            renameat_args->olddirfd, renameat_args->oldpath, renameat_args->newdirfd,
            renameat_args->newpath);
        break;
    case (SYS_umask):;
    ...
    case (SYS_setsockopt):;
        ...
        ret                                   = setsockopt(
            setsockopt_args->socket, setsockopt_args->level, setsockopt_args->option_name,
    ...
    case (SYS_connect):;
        ...
        ret                             = connect(
            connect_args->sockfd, (struct sockaddr*)&connect_args->addr, connect_args->addrlen);
        break;
    case (SYS_bind):;
    ...
    case (SYS_accept):;
        ...
        ret                           = accept(
            accept_args->sockfd, (struct sockaddr*)&accept_args->addr, &accept_args->addrlen);
        break;
    case (SYS_recvfrom):;
        ...
        ret                = recvfrom(
            recvfrom_args->sockfd, recvfrom_args->buf, recvfrom_args->len, recvfrom_args->flags,
            src_addr, addrlen);
        break;
    case (SYS_sendto):;
        ...
        ret                    = sendto(
            sendto_args->sockfd, sendto_args->buf, sendto_args->len, sendto_args->flags, dest_addr,
            dest_addrlen);
        break;
    case (SYS_sendfile):;
...
void protected_incoming_syscall(struct edge_call* edge_call) {
    struct edge_syscall* syscall_info;
    size_t size;
    uintptr_t args;
    uintptr_t call_ret;
    ...
    {
        int ret;
        ret = adm_get_region(ADM_UID_SYSCALL_META, (uintptr_t*)&syscall_info, &size);
    ...
    if ((0xf000 & syscall_info->syscall_num) != 0x0) { // calls with direct ptr to adm
        ...
        case (SYS_openat):;
            ...
            if (!openpath)
                goto syscall_error;
            ret =
                openat(openat_args->dirfd, (char*)openpath, openat_args->flags, openat_args->mode);
            break;
        case (SYS_unlinkat):;
            ...
            if (!unlinkpath)
                goto syscall_error;
            ret = unlinkat(unlinkat_args->dirfd, (char*)unlinkpath, unlinkat_args->flags);
            break;
        case (SYS_fstatat):;
            if (!write_permitted)
                goto syscall_error;
            ...
            struct stat* fstatat_stats        = (struct stat*)adm2ptr(&fstatat_args->stats);
            if (!fstatpath || !fstatat_stats)
                goto syscall_error;
            ret =
                fstatat(fstatat_args->dirfd, (char*)fstatpath, fstatat_stats, fstatat_args->flags);
            break;
        case (SYS_fstat):;
            if (!write_permitted)
                goto syscall_error;
        ...
        case (SYS_epoll_ctl):;
            ...
            struct epoll_event* event = (struct epoll_event*)adm2ptr(&epoll_ctl_args->event);
            if (!event)
                goto syscall_error;
            ret = epoll_ctl(epoll_ctl_args->epfd, epoll_ctl_args->op, epoll_ctl_args->fd, event);
            break;
        case (SYS_epoll_pwait):;
            if (!write_permitted)
                goto syscall_error;
            sargs_SYS_epoll_pwait_p* epoll_pwait_args = (sargs_SYS_epoll_pwait_p*)args;
            struct epoll_event* events = (struct epoll_event*)adm2ptr(&epoll_pwait_args->events);
            if (!events)
                goto syscall_error;
            ret = epoll_wait(
        ...
        case (SYS_renameat2):;
            ...
            if (!oldpath || !newpath)
                goto syscall_error;
            ret = renameat(renameat_args->olddirfd, oldpath, renameat_args->newdirfd, newpath);
            break;
        case (SYS_write):;
        ...
        case (SYS_setsockopt):;
            ...
            ret = setsockopt(
                setsockopt_args->socket, setsockopt_args->level, setsockopt_args->option_name,
                (void*)optval_buf, setsockopt_args->option_len);
            break;
        case (SYS_connect):;
            sargs_SYS_connect_p* connect_args = (sargs_SYS_connect_p*)args;
            struct sockaddr* connect_addr     = (struct sockaddr*)adm2ptr(&connect_args->addr);
            if (!connect_addr)
                goto syscall_error;
            ret = connect(connect_args->sockfd, connect_addr, connect_args->addrlen);
            break;
        case (SYS_bind):;
            sargs_SYS_bind_p* bind_args = (sargs_SYS_bind_p*)args;
            struct sockaddr* bind_addr  = (struct sockaddr*)adm2ptr(&bind_args->addr);
            if (!bind_addr)
                goto syscall_error;
        ...
        case (SYS_accept):;
            ...
            struct sockaddr* accept_addr    = (struct sockaddr*)adm2ptr(&accept_args->addr);
            socklen_t* accept_addrlen       = (socklen_t*)adm2ptr(&accept_args->addrlen);
            if (!accept_addr || !accept_addrlen)
                goto syscall_error;
        ...
        case (SYS_recvfrom):;
            ...
            struct sockaddr* recv_addr = recvfrom_args->src_addr_is_null
                                             ? NULL
                                             : (struct sockaddr*)adm2ptr(&recvfrom_args->src_addr);
            socklen_t* recv_addrlen    = recvfrom_args->src_addr_is_null
                                             ? NULL
            ...
            ret = recvfrom(
                recvfrom_args->sockfd, (void*)recv_buf, recvfrom_args->len, recvfrom_args->flags,
        ...
        case (SYS_sendto):;
            ...
            struct sockaddr* dest_addr = sendto_args->dest_addr_is_null
                                             ? NULL
            ...
            ret = sendto(
                sendto_args->sockfd, (void*)send_buf, sendto_args->len, sendto_args->flags,
        ...
        case (SYS_fcntl):;
            ...
            if (!cmd_args_buf)
                goto syscall_error;
            if (!fcntl_args->has_struct)
                ret = fcntl(fcntl_args->fd, fcntl_args->cmd, *(unsigned long*)cmd_args_buf);
            else
        ...
        case (SYS_getpeername):;
            if (!write_permitted)
                goto syscall_error;
            sargs_SYS_getpeername_p* getpeername_args = (sargs_SYS_getpeername_p*)args;
            struct sockaddr* getpeername_addr = (struct sockaddr*)adm2ptr(&getpeername_args->addr);
            socklen_t* getpeername_addrlen    = (socklen_t*)adm2ptr(&getpeername_args->addrlen);
            if (!getpeername_addr || !getpeername_addrlen)
                goto syscall_error;
            ret = getpeername(getpeername_args->sockfd, getpeername_addr, getpeername_addrlen);
            break;
        case (SYS_getsockname):;
            if (!write_permitted)
                goto syscall_error;
            sargs_SYS_getsockname_p* getsockname_args = (sargs_SYS_getsockname_p*)args;
            struct sockaddr* getsockname_addr = (struct sockaddr*)adm2ptr(&getsockname_args->addr);
            socklen_t* getsockname_addrlen    = (socklen_t*)adm2ptr(&getsockname_args->addrlen);
            if (!getsockname_addr || !getsockname_addrlen)
                goto syscall_error;
            ret = getsockname(getsockname_args->sockfd, getsockname_addr, getsockname_addrlen);
            break;
        case (SYS_pselect6):;
            sargs_SYS_pselect_p* pselect_args = (sargs_SYS_pselect_p*)args;
            fd_set* readfds =
                pselect_args->readfds_is_null ? NULL : (fd_set*)adm2ptr(&pselect_args->readfds);
            fd_set* writefds =
                pselect_args->writefds_is_null ? NULL : (fd_set*)adm2ptr(&pselect_args->writefds);
            fd_set* exceptfds =
                pselect_args->exceptfds_is_null ? NULL : (fd_set*)adm2ptr(&pselect_args->exceptfds);
            struct timespec* timeout = pselect_args->timeout_is_null
                                           ? NULL
                                           : (struct timespec*)adm2ptr(&pselect_args->timeout);
            sigset_t* sigmask =
                pselect_args->sigmask_is_null ? NULL : (sigset_t*)adm2ptr(&pselect_args->sigmask);
            ret = pselect(pselect_args->nfds, readfds, writefds, exceptfds, timeout, sigmask);
            break;
        default:
            goto syscall_error;
        ...
        case (SYS_openat):;
            ...
            ret                           = openat(
                openat_args->dirfd, openat_args->path, openat_args->flags, openat_args->mode);
            break;
        case (SYS_unlinkat):;
        ...
        case (SYS_sendto):;
            ...
            socklen_t dest_addrlen = sendto_args->dest_addr_is_null ? 0 : sendto_args->addrlen;
            ret                    = sendto(
                sendto_args->sockfd, sendto_args->buf, sendto_args->len, sendto_args->flags,
                dest_addr, dest_addrlen);
            break;
        case (SYS_sendfile):;
            if (!write_permitted)
        ...
        case (SYS_fcntl):;
            ...
            if (!fcntl_args->has_struct)
                ret = fcntl(fcntl_args->fd, fcntl_args->cmd, *(unsigned long*)fcntl_args->arg);
            else
                ret = fcntl(fcntl_args->fd, fcntl_args->cmd, fcntl_args->arg);
            break;
        case (SYS_getuid):;
