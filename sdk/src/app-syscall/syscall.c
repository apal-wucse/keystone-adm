...
int ocall_share(
    unsigned long call_id, void* retbuf, size_t retbuf_len, uintptr_t type_info, uintptr_t count,
    unsigned long share_type) {
    return SYSCALL_6(
        SYSCALL_OCALL_SHARE, call_id, retbuf, retbuf_len, type_info, count, share_type);
}
...
/* returns sealing key */
int get_sealing_key(
    struct sealing_key* sealing_key_struct, size_t sealing_key_struct_size, void* key_ident,
    size_t key_ident_size) {
    return SYSCALL_4(
        SYSCALL_GET_SEALING_KEY, sealing_key_struct, sealing_key_struct_size, key_ident,
        key_ident_size);
}

unsigned long map_adm(size_t* size) { return SYSCALL_1(SYSCALL_MAP_ADDITIONAL, size); }

uintptr_t read_cycle() { return SYSCALL_0(SYSCALL_READ_CYCLE); }
