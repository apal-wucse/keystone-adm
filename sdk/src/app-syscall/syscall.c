...
ocall_share(
    ...
    uintptr_t count, unsigned long share_type) {
  return SYSCALL_6(
      SYSCALL_OCALL_SHARE, call_id, retbuf, retbuf_len, type_info, count,
      share_type);
}
...
unsigned long
map_adm(size_t* size) {
  return SYSCALL_1(SYSCALL_MAP_ADDITIONAL, size);
}

uintptr_t
read_cycle() {
  return SYSCALL_0(SYSCALL_READ_CYCLE);
}
