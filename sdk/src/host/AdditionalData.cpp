...
bool
AdditionalData::storeBytes(uint8_t* data, size_t size, uintptr_t uid) {
...
bool
AdditionalData::storeVec(DataVec vec, uintptr_t uid) {
  if (storedData.size() >= ADM_SLOT_MAX) {
    return false;
  }
  if (vec.empty()) {
...
size_t
AdditionalData::getStoredSize() {
  return storedData.size();
}

const std::vector<AdditionalData::DataBytes>&
AdditionalData::getStoredData() {
  return storedData;
}

std::vector<AdmTypeInfo>
AdditionalData::genTypeInfo() {
  std::vector<AdmTypeInfo> type_info;
  uintptr_t offset = sizeof(AdmHeader);
  for (DataBytes& d : storedData) {
    type_info.push_back((AdmTypeInfo){d.uid, TYPE_STATIC_ARR_BYTE, d.size,
