...
#define KEYSTONE_IOC_UTM_INIT \
  _IOR(KEYSTONE_IOC_MAGIC, 0x07, struct keystone_ioctl_memalloc)

#define KEYSTONE_IOC_ADM_INIT \
  _IOR(KEYSTONE_IOC_MAGIC, 0x08, struct keystone_ioctl_memalloc)
#define KEYSTONE_IOC_SWAP_MAP \
  _IOR(KEYSTONE_IOC_MAGIC, 0x09, struct keystone_ioctl_memalloc)
...
struct keystone_ioctl_memalloc {
  uintptr_t eid;
  uintptr_t size;
  __u8 mem_type;
  __u8 protect_type;
...
enum keystone_adm_protect_type {
  ADM_DEFAULT_READ = 0,
  ADM_DEFAULT_NOPERM,
};

enum keystone_memtype {
  TRUSTED_MEMORY = 0,
  UNTRUSTED_MEMORY,
  PROTECTED_MEMORY,
};
