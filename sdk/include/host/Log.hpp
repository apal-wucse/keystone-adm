...
class Logger {
  public:
    ...
    inline bool DirectToFile(const std::string& path, bool append = false) {
        ...
        return ResetOutputStream_(
            new std::ofstream{path, append ? std::ios_base::app : std::ios_base::out});
    }
    ...
    template <typename T> inline const Logger& operator<<(T&& to_write) const {
        const std::lock_guard<std::mutex> lock{mtx_};
        if (enabled_) {
...
template <typename T> class Formattable {
  public:
    void Format(std::ostream& os, FormatMethod method = FormatMethod::Default) const {
        switch (method) {
...
template <typename T, FormatMethod M> class DoFormat {
  public:
...
/* Convenience helpers. */
template <typename T> DoFormat<T, FormatMethod::JSON> FormatAsJSON(const T& to_format) {
    return DoFormat<T, FormatMethod::JSON>{to_format};
}

template <typename T> DoFormat<T, FormatMethod::Pretty> FormatAsPretty(const T& to_format) {
    return DoFormat<T, FormatMethod::Pretty>{to_format};
}

template <typename T> DoFormat<T, FormatMethod::Default> FormatAsDefault(const T& to_format) {
    return DoFormat<T, FormatMethod::Default>{to_format};
}

template <typename T> DoFormat<T, FormatMethod::Default> Format(const T& to_format) {
    return DoFormat<T, FormatMethod::Default>{to_format};
}
