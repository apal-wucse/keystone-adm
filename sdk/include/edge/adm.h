...
static inline AdmHeader* adm_get_header() { return _adm_hdr; }

static inline AdmRegionHeader* adm_get_region_header(size_t idx) {
...
static inline void* adm_get_region_base(AdmRegionHeader* region_hdr) {
    if (!region_hdr)
        return NULL;

    /* validate offset to raw data bytes */
    if (region_hdr->offset > UINTPTR_MAX - _adm_start || region_hdr->offset >= _adm_len) {
        return NULL;
    }
    /* validate data size */
...
static inline size_t adm_region_count() { return _adm_hdr ? _adm_hdr->data_n : 0; }

static inline uintptr_t adm2ptr(AdmPtr* admptr) {
    if (admptr->offset > _adm_len)
        return (uintptr_t)NULL;
    return (uintptr_t)(_adm_start + admptr->offset);
}

static inline uintptr_t ptr2adm(void* ptr) {
